...
- **THEN** that beat's content is hidden (`opacity-0`) until the entrance animation begins on the next frame
...
Each beat's primary text SHALL use a continuous gentle letter wave after entrance (current beat) or immediately (previously revealed beats), using the CSS letter-span pattern (Madras Academy style) with word-level wrap boundaries.
...
- **THEN** letters animate in a continuous wave with reduced amplitude (`WAVE_AMPLITUDE_PX = 4`) and longer cycle (`WAVE_CYCLE_DURATION_S = 3.8`) compared to the Madras Academy tutorial defaults
...
- **THEN** `calendar_flip` appears inline to the left of "July 13, 1999", vertically aligned with the date typography at approximately 4em cap height

#### Scenario: Calendar renders sharply on high-DPI screens

- **WHEN** beat 0 is visible on a retina or high-DPI display
- **THEN** the calendar Lottie uses `devicePixelRatio` in its render config so it is not upscaled-blurry

#### Scenario: Single twinkle on date letters

- **WHEN** beat 0 is visible
- **THEN** only `twinkle_stars_02` decorates the date text as a glitter overlay on the glyphs (not a pair of side flanking twinkles)
...
- **THEN** beats 0 through N display at reduced opacity (~0.55) while beat N+1 is at full opacity
...
- **WHEN** beat 2 is revealed
- **THEN** the photo appears in a white/cream frame with a wider bottom border, slight rotation (~−3°), and soft drop shadow (single polaroid, not a pair)
...
- **THEN** the closing copy and emojis appear below the beat 2 identity line in the right column, not as a separate full-width row below the photo
...
- **THEN** the identity line aligns toward the top of the polaroid, the closing line occupies the middle, and emojis align toward the bottom — without shifting beats 0–1 vertically when beat 3 appears
...
- **THEN** the cat Lottie appears to the right of "And that little baby was you, Cila." (not on the left)

#### Scenario: Cat floats gently

- **WHEN** beat 2 is the current beat
- **THEN** the cat Lottie has a subtle continuous float motion (not completely static)
...
- **THEN** the emoji animate with a brief bounce (one or two cycles) and then remain still
...
- **THEN** horizontal overflow is clipped without creating nested vertical scrollbars on beat wrappers

#### Scenario: Mobile beat 3 vertical distribution

- **WHEN** all four beats are visible on a viewport below the `sm` breakpoint
- **THEN** beat sections use vertical space distribution (`justify-evenly`) so content fills the viewport without unnecessary scroll when possible

#### Scenario: Desktop beat 2–3 stable top alignment

- **WHEN** the user advances from beat 2 to beat 3 on a viewport at or above the `sm` breakpoint
- **THEN** beats 0, 1, and the polaroid block retain their vertical position (only the closing content appears in the right column)
...
- **THEN** the in-slide CTA label reads "Continue" (no countdown suffix while locked)
