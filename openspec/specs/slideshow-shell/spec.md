...
Fullscreen Slideshow container that displays one Slide at a time, supports modular slide registration, smooth transitions, Intro → Birth Story → Balloon Game → Gallery → Gift catch → Farewell flow, and shell-level audio control.
...
#### Scenario: Gift catch to farewell transition

- **WHEN** navigation from Gift catch slide to Slide 6 Farewell is triggered
- **THEN** the Gift catch slide exits with a smooth animation before the Farewell slide enters
